...
def get_all_jobs_from_views():
    jobs_list = []
    views_resp = requests.get(f"{JENKINS_URL}/api/json", auth=auth).json()
    for view in views_resp.get('views', []):
        view_name = view['name']
        view_jobs_resp = requests.get(f"{JENKINS_URL}/view/{view_name}/api/json", auth=auth).json()
...
def retry_job(view, job_name):
    ...
    if retry_counts[job_name] <= 3:
        requests.post(f"{JENKINS_URL}/view/{view}/job/{job_name}/build", auth=auth)
        return f"Triggered retry for {job_name} (Attempt {retry_counts[job_name]}/3)"
    else:
        return f"Max retries reached for {job_name}"

def get_nodes():
    nodes_resp = requests.get(f"{JENKINS_URL}/computer/api/json", auth=auth).json()
    nodes_list = []
    for node in nodes_resp.get('computer', []):
        nodes_list.append({
            ...
            "last_connected": "-"  # Jenkins API doesn't directly give this, placeholder
        })
    return nodes_list
...
def reconnect_node(node_name):
    ...
    if node_retry_counts[node_name] <= 3:
        # Normally you'd use Jenkins CLI or a script for reconnect.
